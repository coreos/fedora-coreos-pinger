...
AFTERBURN_AWS_INSTANCE_ID=i-0e12a3451176181c4"#.to_string()
}
...
fn test_minimal_with_file() {
    ...
    let mut karg_file = tempfile::NamedTempFile::new().expect("Unable to create temporary karg file");
    let mut aleph_version_file = tempfile::NamedTempFile::new().expect("Unable to create temporary aleph version file");
    let mut metadata_file = tempfile::NamedTempFile::new().expect("Unable to create temporary metadata file");

    writeln!(karg_file, "{}", cmdline).expect("Unable to write to temporary karg file");
    writeln!(aleph_version_file, "{}", original_os_version).expect("Unable to write to temporary aleph version file");
    writeln!(metadata_file, "{}", metadata).expect("Unable to write to temporary metadata file");

    let minimal_id = minimal::IdentityMin::collect_minimal_data(karg_file.path().to_str().unwrap(),
    ...
    karg_file.close().expect("Unable to close the temporary karg file");
    aleph_version_file.close().expect("Unable to close the temporary aleph version file");
    metadata_file.close().expect("Unable to close the temporary metadata file");
}
