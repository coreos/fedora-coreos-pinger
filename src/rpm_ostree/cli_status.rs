...
/// Note: Returns vector instead of btree as in Afterburn
pub fn local_deployments() -> Fallible<Vec<Release>> {
    let status = status_json(false)?;
    let mut deployments = Vec::<Release>::new();
    for entry in status.deployments {
        let release = entry.into_release();
        deployments.push(release);
    }
    Ok(deployments)
}
